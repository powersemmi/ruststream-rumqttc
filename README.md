...
- **Pages for `&[T]` handlers.** A PUBLISH packet carries one message, so the crate assembles pages on the client to the size the mount site named (`b.include(ingest.batch(nonzero!(64)))`), closing a partial page 20 ms after its first delivery. Nothing at the mount site says which side of the wire filled it.
